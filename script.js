...
function calculateMolecule(formula) {
  ...
  chemArray.forEach(x => {
    if (elementAndNumber.test(x)) {
      ele = x.match(element)[0]
      elemNumMatch = x.match(numberAfterElement)
      elemNumMatch ? atoms = parseInt(elemNumMatch[0]) : atoms = 1
      elmass[level][ele] = (elmass[level][ele] || 0)+atoms
    }
    else if (openingParenthesis.test(x)) {
      elmass[++level] = {}
    }
    else if (closingParenthesis.test(x)) {
      parenNumMatch = x.match(/\d+$/)
      parenNumMatch ? parenNum = parseInt(parenNumMatch[0]) : parenNum = 1
      for ([ele, atoms] of Object.entries(elmass[level])) {
        elmass[level - 1][ele] = (elmass[level - 1][ele] || 0) + atoms * parenNum
      }
      elmass[level--] = null
    }
  })
